# specs/ — bundled contracts for Local-First Art Collective
...
This directory was bundled on **2026-05-09T12:59:07Z** from `kody-w/RAPP @ fb784f9 (post-Bond-Pulse)`. The contracts here are FROZEN at planting time — they do not change unless an operator runs a `specs refresh` action.
...
| `SUBMISSION_PROTOCOL.md` | The kind-specific protocol — what `neighborhood` neighborhoods uniquely traffic in |
...
- **Lifted at:** 2026-05-09T12:59:07Z
...
```bash
# (future) brainstem run specs-refresh --neighborhood local/local-art-collective
```
